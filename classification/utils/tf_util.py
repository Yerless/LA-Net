...
def Residual_Attention_Block(
    ...
    with tf.variable_scope(scope) as sc:
        ...
        stride_h, stride_w = stride
        #----------------------------feature-------------------------------
        outputs1 = tf.nn.conv2d(
            inputs, kernel, [1, stride_h, stride_w, 1], padding=padding
        )
        biases1 = _variable_on_cpu(
            "biases1", [num_output_channels], tf.constant_initializer(0.0)
        )
        feature1 = tf.nn.bias_add(outputs1, biases1)
        #-----------------------------score--------------------------------
        outputs2 = tf.nn.conv2d(
            inputs, kernel, [1, stride_h, stride_w, 1], padding=padding
        )
        biases2 = _variable_on_cpu(
            "biases2", [num_output_channels], tf.constant_initializer(0.0)
        )
        feature2 = tf.nn.bias_add(outputs2, biases2)
        # channel1 = feature2.get_shape()[0].value
        # channel2 = feature2.get_shape()[1].value
        # channel3 = feature2.get_shape()[2].value
        # channel4 = feature2.get_shape()[3].value
        #remove_dimension=1
        feature2=tf.squeeze(feature2)
        #swap_channels
        feature2=tf.transpose(feature2,[0,2,1])
        score=tf.nn.softmax(feature2)
        score=tf.transpose(score,[0,2,1])
        score=tf.expand_dims(score,2)
        attention_feature=feature1 * score
        # new_feature2=tf.reshape(feature,(channel1,-1))
        # feature_score=tf.nn.softmax(new_feature)
        # score=tf.reshape(feature_score,(channel1,channel2,channel3,channel4))
        # attention_feature=feature * score
        if(num_in_channels!=num_output_channels):
            inputs = tf.nn.conv2d(
            inputs, kernel, [1, stride_h, stride_w, 1], padding=padding
        )
            attention_output=tf.add(attention_feature,inputs)
        else:
            attention_output = tf.add(attention_feature, inputs)
        if bn:
             attention_output= batch_norm_for_conv2d(
                attention_output, is_training, bn_decay=bn_decay, scope="bn"
            )
        if activation_fn is not None:
            attention_output = activation_fn(attention_output)
